...
try:
    ...
    if filename is not None and not (filename.startswith('generate_')
        ...
        run_python_file(os.path.join(PWB_DIR, 'generate_user_files.py'),
...
def main():
    ...
    # always run the login script first to make sure that we're logged in
    filename_log = 'scripts/login.py'
    file_package_log = os.path.dirname(filename_log).replace(os.sep, '.')
    run_python_file(filename_log, [filename_log], [Path(filename_log).stem], file_package_log)
